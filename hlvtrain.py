...
def train(hyp, opt, device, callbacks):  # path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(
        ...
        # Update image weights (optional, single-GPU only)RANK
        ...
        for i, (
            ...
            if RANK in [-1, 0]:
                ...
                pbar.set_description(("%10s" * 1) % (f"{epoch}/{epochs - 1}"))
...
def main(opt, callbacks=Callbacks()):
    ...
    # Resume
    if (
        opt.resume and not check_comet_resume(opt) and not opt.evolve
